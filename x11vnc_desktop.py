...
def parse_args(description):
    ...
    parser.add_argument('-o', '--owner',
                        help='The owner to pull image from. ' +
                        'The default is x11vnc.',
                        default='x11vnc')
    ...
    parser.add_argument('-t', '--tag',
                        help='Tag of the image. The default is zh_CN. ' +
    ...
    parser.add_argument('--password',
                        help='Specify a password for VNC instead of 123. ' +
...
if __name__ == "__main__":
    ...
    args.image = args.owner + args.image
    ...
    volumes = ["-v", pwd + ":" + docker_home + "/"+default_name,
    ...
    cmd = ["docker", "run", "-d", "--name", hostname,
                     "--shm-size", "2g", "-p", port_http + ":6080",
                     "-p", port_vnc + ":5900"] + \
        envs + volumes + args.args.split() + \
        ['--security-opt', 'seccomp=unconfined', '--cap-add=SYS_PTRACE','--gpus','all',
